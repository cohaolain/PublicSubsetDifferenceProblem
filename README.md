# Subset Difference Problem
![alt text](assets/img/mathsDef.png "Mathematical Definition")

This is the front-end for submitting answers via a smart contract to the blockchain for the above problem.

Essentially, imagine you've been given a box of integer weights. Find the best way to balance both weights on a set of balance scales such that the difference between the weight of the two sides is minimal.

If the problem set was {5,3,9}, you could put {5,3} on one side of the scales (in one subset), and {9} on the other (in the other subset). This would give a difference of 1 (between the sums of the subsets). This would be the optimal solution for this problem set (provable for this small set by listing all possible subsets and the sum differences they produce).
